...
A train service has a ``setting'' (in the sense of DUL) which can be
understood as a ``circumstance'', and this is a ``static section'' of
the train journey.

\emph{Note: This is a slight over-simplification, since passenger trains
for instance sometimes benefit from cleaning processes while running. }
...
\hypertarget{false-simplifications-true-mess}{%
\subsubsection{False simplifications = True
mess}\label{false-simplifications-true-mess}}

The user is free to define distinct services or a single combined one,
but the latter option is not recommended (customs clearance may be
provided while some maintenance operations may need to be delayed): what
may first appear as a simplification may result in a mess.
